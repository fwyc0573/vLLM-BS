"""Load the installed vLLM core extension for source-tree runs."""
...
def _required_extension_path() -> Path:
    ...
    extension_path = Path(package_dir) / "_C.abi3.so"
    if not extension_path.is_file():
        raise ImportError(f"Required vLLM core extension not found: {extension_path}")
...
if _spec is None or _spec.loader is None:
    raise ImportError(f"Cannot load vLLM core extension: {_extension_path}")
