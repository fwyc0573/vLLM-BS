#!/bin/bash
# Test: TP=2 + EP MoE Parallel Configuration
# Description: Test P2pNcclConnector with Tensor Parallel size 2 + Expert Parallel on MoE model
# Expected Result: Success - EP requires TP*DP > 1, TP=2 satisfies this
# GPU Allocation: Prefill=[1,2], Decode=[3,4] (avoiding occupied GPU 0)
...
OUTPUT_LOG="$SCRIPT_DIR/logs/test_tp2_ep_moe_output.log"

# Test-specific configuration
TEST_NAME="TP=2 + EP MoE Test"
...
PREFILL_CACHE_DIR="$SCRIPT_DIR/vllm_cache_prefill_tp2_ep"
DECODE_CACHE_DIR="$SCRIPT_DIR/vllm_cache_decode_tp2_ep"
...
PREFILL_INDUCTOR_CACHE_DIR="$SCRIPT_DIR/inductor_cache_prefill_tp2_ep"
DECODE_INDUCTOR_CACHE_DIR="$SCRIPT_DIR/inductor_cache_decode_tp2_ep"
...

{
    ...
    # Run the disaggregated prefill-decode test with TP=2 + EP
...
PY
    ...
    SYNC_FILE="$SCRIPT_DIR/prefill_done_tp2_ep_$.marker"
    rm -f "$SYNC_FILE"
    ...
    # Start prefill (producer) in background with TP=2 + EP
    if [[ $ENABLE_PROFILE -eq 1 ]]; then
    ...
    # Start decode (consumer) in background with TP=2 + EP
    set +e
    if [[ $ENABLE_PROFILE -eq 1 ]]; then
