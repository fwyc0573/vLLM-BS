#!/bin/bash
# Test: TP=2 MoE Parallel Configuration
# Description: Test P2pNcclConnector with Tensor Parallel size 2 on MoE model
# Expected Result: Success - P2pNcclConnector supports symmetric TP
...
OUTPUT_LOG="$SCRIPT_DIR/logs/test_tp2_moe_output.log"

# Test-specific configuration
TEST_NAME="TP=2 MoE Test"
...
PREFILL_CACHE_DIR="$SCRIPT_DIR/vllm_cache_prefill_tp2"
DECODE_CACHE_DIR="$SCRIPT_DIR/vllm_cache_decode_tp2"
...
PREFILL_INDUCTOR_CACHE_DIR="$SCRIPT_DIR/inductor_cache_prefill_tp2"
DECODE_INDUCTOR_CACHE_DIR="$SCRIPT_DIR/inductor_cache_decode_tp2"
...

{
    ...
    # Run the disaggregated prefill-decode test with TP=2
...
PY
    ...
    SYNC_FILE="$SCRIPT_DIR/prefill_done_tp2_$.marker"
    DECODE_READY_FILE="$SCRIPT_DIR/decode_ready_tp2_$.marker"
    rm -f "$SYNC_FILE" "$DECODE_READY_FILE"
    ...
    cleanup() {
        ...
        rm -f "$SYNC_FILE" "$DECODE_READY_FILE" || true
    }
    trap cleanup EXIT

    echo "KV_BASE_PORT: $KV_BASE_PORT"
    echo "SYNC_FILE: $SYNC_FILE"
    echo "DECODE_READY_FILE: $DECODE_READY_FILE"
    if [[ $ENABLE_PROFILE -eq 1 ]]; then
        echo "Profiler traces: $VLLM_TORCH_PROFILER_DIR"
    fi
    echo ""

    # Start prefill (producer) in background with TP=2
    if [[ $ENABLE_PROFILE -eq 1 ]]; then
    ...
    # Start decode (consumer) in background with TP=2
    set +e
    if [[ $ENABLE_PROFILE -eq 1 ]]; then
